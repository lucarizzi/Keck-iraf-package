...
#	2003-Sep-22		GDW		Fixed botched enhancements
#-----------------------------------------------------------------------
...
bool	autogen	{ yes, prompt="Generate box file from DesiSlits?" }
bool    doxbox  { yes, prompt="Launch xbox when done?" }
real	z1		{ 1000, prompt="minimum greylevel to be displayed" }
real	z2		{ 10000, prompt="maximum greylevel to be displayed" }
struct  *ilist
struct  iline
...
begin
	...
	if( access(file1)){
		printf( "verified access to box file %s\n", file1)
		type( file1)
	} else {
		printf( "creating empty box file %s\n", file1)
	    type( "", >file1)
    }
    ...
    while( yes ){
        ...
        if( display){
			...
			dmosdisplay( i_image, frame, zscale-, zra-, z1=z1, z2=z2)
        }
