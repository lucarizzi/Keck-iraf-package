define	DEF_PROP_FILE	"proto$lib/prop/redprop.txt"	# default properties
...
# 20feb03 -- modified to output order as well, in cases of multiple orders
# 25feb03 -- added kludge to pt_xfm to guard against asin problems, large beta
...
# Store values in params:
	call eval_on_axis (e1, a2, as, a3, a4, sys)

	call trace (e1, a2, as, a3, a4, ccd, sys)

## Following is the older version
##	call full_trace (fda, e1, a2, as, a3, a4, ccd, sys)

##	call close (fda)

end

procedure	setup (e1, a2, as, a3, a4, ccd, sys)
...
double	cz, sz			# cos, sin in theta-z, slider axle
double	cy, sy			# cos, sin in theta-y, slider axle
...
# SLIDER: The slider axle actually has more error than individual gratings, so
# it makes sense to break this out as a separate transform.
# Only tilts in y and z need be considered (the axle is aligned with the x-axis)
	cz = cos (SL_ZERR(sys))
	sz = sin (SL_ZERR(sys))
	cy = cos (SL_YERR(sys))
	sy = sin (SL_YERR(sys))
...
procedure	full_trace (fda, e1, a2, as, a3, a4, ccd, sys)
...
# Ready to loop through:
	while (fscan (fda) != EOF) {
		...
		call pt_xfm (x, y, w, e1, a2, as, a3, a4, ccd, sys, xpix, ypix, n)
...
procedure	pt_xfm (xmm, ymm, wave, e1, a2, as, a3, a4, ccd, sys, xpix, ypix, n)
...
# transform into slider system
	call gen_xfm (r, as, YES)

# transform into grating system
...
# TRAP PROBLEMS ...
	beta = (ORDER(sys) * GRLINES(sys) * abs (wave) / cos (gamma)) - sin (alpha)
	if (abs (beta) > 1.d0) {
		call eprintf (0, "DEBUG: asin (>1)")
	}
	beta = max (beta, -0.5d0)	# kludge, but well off detector
	beta = min (beta,  0.5d0)	# kludge, but well off detector
...
#	call grat_to_ics (beta, gamma, wave, sys, a4, a3, xpix, ypix)
	call grat_to_ics (beta, gamma, wave, sys, a4, a3, as, xpix, ypix)
...
procedure trace (e1, a2, as, a3, a4, ccd, sys)
...
call eprintf ("FIX! WRONG! no alpha/beta! \n")
...
call eprintf ("\nOn-axis values: beta =%8.4f ==> lambda(1) =%8.2f; lambda(2) =%8.2f\n")
...
do i = 0, nslit-1 {
	call pt_xfm (Memd[bufxmm+i], Memd[bufymm+i],
		Memd[bufwav], e1, a2, as, a3, a4, ccd, sys,
...
call eprintf ("processing %d (index=%d):  %7.2f %2.0f\n")
call pargi (j)
call pargi (index)
...
# added 01/09/05 -- if ghost, check that primary is on chip:
			if (Memd[bufwav+j] < 0.D0) {
			    call pt_xfm (Memd[bufxmm+i], Memd[bufymm+i],
				-1.D0*Memd[bufwav+j], e1, a2, as, a3, a4, ccd, sys,
				Memr[bufx+i], Memr[bufy+i], n)
			...
			call pt_xfm (Memd[bufxmm+i], Memd[bufymm+i],
				Memd[bufwav+j], e1, a2, as, a3, a4, ccd, sys,
				Memr[bufx+i], Memr[bufy+i], n)
			call ics_to_ccd (Memr[bufx+i], Memr[bufy+i], ccd, n,
					Memr[bufx+i], Memr[bufy+i], stat)
			Memi[bufc+i] = n
...
#	call fprintf (fdb, "# xmm ymm   xpix ypix    ID\n")
			...
			if (Memr[bufx+i] == INDEF)
				next
			call fprintf (fdb, "%03d %8.3f %8.3f %8.2f %2.0f   %7.2f %7.2f \n")
				call pargi (index)
				call pargd (Memd[bufxmm+i])
				call pargd (Memd[bufymm+i])
				call pargd (Memd[bufwav+j]*1.D4)
...
# Write the setup info:
	call fprintf (fdb, "# %03d %s %7.3f  %6.3f %6.3f  %6.1f %3.0f %4d\n")
		call pargi (index)
		call pargstr (image)
...
procedure	proof (xmm, ymm, nslit, wave, nwave, fdb, e1, a2, as, a3, a4, ccd, sys)
...
# Allocate these vectors
	...
	do j = 1, nwave {
	    do i = 1, nslit {
		call pt_xfm (xmm[i], ymm[i], wave[j], e1, a2, as, a3, a4, ccd, sys, xpix, ypix, n)
		call ics_to_ccd (xpix, ypix, ccd, n, xpix, ypix, stat)
...
procedure	eval_on_axis (e1, a2, as, a3, a4, sys)
...
begin
	...
	call pre_grating (xslit, yslit, e1, a2, as, sys, r)
...
# Apply the grating equation
	...
	call clputd ("alpha0", RADTODEG(alpha))
	call clputd ("beta0",  RADTODEG(beta))
	call clputd ("wave0", wave*1.d4)
	call clputr ("anamf0", anamf)
