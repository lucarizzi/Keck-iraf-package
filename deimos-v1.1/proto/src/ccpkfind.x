...
# fit parabola to pk, using weighted averages
	    ...
	    call ppf2(Memr[bufsect], ned*2+1, xoff, yoff)
# repeat with new xoff,yoff
	    call ppf2(Memr[bufsect], ned*2+1, xoff, yoff)
...
# OK, we now have peak pixel; extract box around peak
	    bufsect = imgs2r (im, ipk-ned, ipk+ned, 1, 1)

# fit parabola to pk, using weighted averages
	    xoff = 0.

	    call ppf1(Memr[bufsect], ned*2+1, xoff)
# repeat with new xoff
	    call ppf1(Memr[bufsect], ned*2+1, xoff)
...
procedure	ppf2 (zarray, n, xoff, yoff)
