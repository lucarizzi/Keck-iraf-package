...
char    binning[SZ_FITS]                         # CCD binning factor
...
#
#  ... Check for binning here
#
        if (imaccf (im, "BINNING")) {
           call imgstr (im, "BINNING", binning, SZ_FITS)
           if (streq (binning, "1,1")) {
              binx = 1
              biny = 1
           } else if (streq (binning, "1,2")) {
              binx = 1
              biny = 2
           } else if (streq (binning, "2,2")) {
              binx = 2
              biny = 2
           } else {
              binx = 1
	...
	call eprintf ("Using BINNING %d %d \n")
		call pargi(binx)
		call pargi(biny)
...
# Assign the proper rotation center for the PO
        if (streq (poname, "LRIS")) {
				...
				call printf ("Using PICKOFF pointing origin\n")
                xrot = 1970
                yrot = 1546
        } else {
                if (strne (poname, ""))
...
# Note -- unsure if better to subtract xoff,yoff (negligible if small)
                if (stat == true) {
                        Memr[xbuf1+ndx] = xstar*binx - prepix
                        Memr[ybuf1+ndx] = ystar * biny
			if (xstar*binx >xgap) {
			    Memr[xbuf1+ndx] = xstar*binx - prepix + 96.627+0.00162*ystar*biny
                            Memr[ybuf1+ndx] = ystar*biny + 2.982
			}
                        Memr[xbuf2+ndx] = xb * binx - prepix
                        Memr[ybuf2+ndx] = yb * biny
			if (xb*binx >xgap) {
			    Memr[xbuf2+ndx] = xb*binx - prepix + 96.627+0.00162*yb*biny
                            Memr[ybuf2+ndx] = yb*biny + 2.982
			}
                        ndx = ndx + 1
                }
            }
...
# ON TO ALIGNING: setup
        nx = ncols * binx               # TMP: should fix; also symmetrize
        ny = nlines * biny              # TMP   ... maskalign
