...
#	2009-Jun-08	GDW	Adapt for both multi-HDU and simple
#-----------------------------------------------------------------------
...
begin
	...
	if( interactive){
		...
		# convert to simple image if needed...
		count_hdu( im, verb+) | scan(nhdu)
		if ( nhdu == 0){

			# image already simple; simply transfer filename...
			simpleim = im

		} else {

			# convert to simple...			
			print( "Converting image to simple style...")
			simpleim = mktemp( tmpprefix) // ".fits"
			multi2simple( im, simpleim, over+)
		}

		# get image size...
		hselect( simpleim, "naxis1,naxis2", "yes") | scan( nx, ny)

		# plot rows/columns...
		print( "\nPlotting cut through image.  Please use the graphics window to mark the\nilluminated region(s).")
		row1 = 1
		row2 = ny
		prows( simpleim, row1, row2, wcs="logical", wx1=0., wx2=0.,
	...
	while( fscan( list, im) != EOF) {
		...
		if ( nhdu == 0){
			...
			ccdhedit( simpleim, keyword, value, extname="", type="real")

			# add image to temp image list for later deletion...
			print( simpleim, >>tmpimlist)

		}

		# add image to list...
		print( simpleim, >>simpleimlist)
	}
	...
	for ( i=1 ; i<=n_regions ; i=i+1){
		...
		specfocus( "@"//simpleimlist, focus=keyword, corwidth=20,
