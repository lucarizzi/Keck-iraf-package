...
# Read in the mask outlines (also defines window params)
	...
	FPWIN_X(mdat) = SIM_FLD_CENX
	FPWIN_Y(mdat) = SIM_FLD_CENY
	FPWIN_W(mdat) = SIM_FLD_WIDTH
	FPWIN_Z(mdat) = SIM_ZSENSE

# Open the graphics stream
	gp = gopen ("stdgraph", NEW_FILE, STDGRAPH)

#	call fp_layout (gp, mdat, indat, tdat, ntarg, sdat, nslit, nlist, SIM_LBLAX1, SIM_LBLAX2, SIM_LBL_VIEW)
	call fp_layout (gp, mdat, indat, tdat, ntarg, sdat, nslit, nlist)
	if (sdat != 0 && nslit > 0) {
		do i = 0, nslit-1
...
# start the interactive stuff
	...
	cosp = cos (PA_FLD(indat))
	sinp = sin (PA_FLD(indat))
...
# translations  XXX there's a lot of excess calc's going on here
		...
		case 'p':
			PA_FLD(indat) = PA_FLD(indat) + anginc
			if (PA_FLD(indat) > PI)
				PA_FLD(indat) = PA_FLD(indat) - 2.*PI
...
call eprintf ("NO projection\n")
		...
		case 'z':
			if (req_valr ("zoom_factor:", zfact, zfact, -5., 80.) == ERR)
				next
			if (zfact == 0.) {
				FPWIN_X(mdat) = SIM_FLD_CENX
				FPWIN_Y(mdat) = SIM_FLD_CENY
				...
				FPWIN_Y(mdat) = 340.
				FPWIN_W(mdat) = SIM_FLD_WIDTH / 2.4
			} else if (zfact == -2.) {
				...
				FPWIN_Y(mdat) = 195.
				FPWIN_W(mdat) = SIM_FLD_WIDTH / 3.
			} else if (zfact > 0.) {
				FPWIN_X(mdat) = wx
				FPWIN_Y(mdat) = wy
				FPWIN_W(mdat) = SIM_FLD_WIDTH / zfact
			}
...
#	call gsview (gp, (0.5-0.49*aspect), (0.5+0.49*aspect), 0.01, 0.99)
	...
	if (reflyax < 0) {
		call gswind (gp, gx1, gx2, gy2, gy1)
	} else {
...
# clear area ...
	...
	xarea[2] = xarea[1]         ;   yarea[2] = 0.17 * gszy + gy1
	xarea[3] = 0.62 * gszx + gx1;   yarea[3] = yarea[2]
...
# put in axes and labels (X refers to display X)
	...
	call grdraw (gp, axlen, 0.)
	call gtext (gp, x0+1.2*axlen, y0, "+X", "h=l;v=c;q=h;s=0.8;col=5")
	call gamove (gp, x0, y0)
	call grdraw (gp, 0., axlen)
	call gtext (gp, x0, y0+1.2*axlen, "+Y", "h=c;v=c;q=h;s=0.8;col=5")
	x =  x0+0.05*gszy
	y =  0.03 * gszy + gy1
	if (reflyax < 0)
	    call gtext (gp, x, y, "Seen From Front", "h=c;v=c;q=h;s=0.9;col=5")
	else
	    call gtext (gp, x, y, "Seen From Back", "h=c;v=c;q=h;s=0.9;col=5")
...
# Draw the compass rose
	...
	call gamove (gp, x0, y0)
	cosp = cos (-PAR_ANG(indat)+PA_FLD(indat)) 
	sinp = sin (-PAR_ANG(indat)+PA_FLD(indat))
	x =  axlen * cosp / 1.		# PL_XMAG(limit)
	y =  -axlen * sinp * reflyax
	call grdraw (gp, x, y)
	...
	y = axlen * cosp * reflyax
	call grdraw (gp, x, y)
	call sprintf (pafld, SZ_ID, "PA=%6.1f")
		call pargd (RADTODEG(PA_FLD(indat)))
	call gtext (gp, x0, y0-1.4*axlen, pafld, "h=c;v=c;q=h;s=0.8;col=5")

# Dispersion Diagram
	call gseti (gp, G_PLCOLOR, CYAN)
	szasec = 0.05 * gszy
	swid = szasec * DEF_SLWID(indat)
	y0 = 0.5 * (yarea[2] + yarea[4])
	x0 = xarea[3] - 0.28 * gszy
...
# assign appropriate color
	...
	if (SEL(tdat,i) == YES) {
		...
		call gseti (gp, G_FACOLOR, WHITE)	# TMP XXX TEST
	}
	...
	if (PCODE(tdat,i) == CODE_AS) {
		...
		if (SEL(tdat,i) == YES)
		    call gmark (gp, x, y, GM_BOX+GM_FILL, -1.*SLWID(tdat,i), -1.*SLWID(tdat,i))	# TMP
		else
