...
#-----------------------------------------------------------------------
string	image		{ prompt="name of DEIMOS alignment image" }
...
begin
	...
	real	fwhm_px
	real	fwhm_asec
    int	    slmskpos	# number of slitmask in use
	int		frame=1
	int		n
	...
	# get box locations...
	hselect( i_image//"[0]", "SLMSKPOS", "yes") | scan( slmskpos)
    if ( nscan() != 1 )
		error( 1, "Can't read SLMSKPOS from image header")
	...
	gkiextract( plotfile, "1-99") \
		| gkimosaic( device="stdgraph", output="", nx=2, ny=2, 
