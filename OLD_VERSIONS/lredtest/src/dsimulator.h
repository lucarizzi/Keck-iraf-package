...
define	PA_FLD		Memd[PTTELDAT($1)+7]	# PA of fld (rad)
define	HA_FLD		Memd[PTTELDAT($1)+8]	# HA of fld (rad)
define	STD_EQX		Memd[PTTELDAT($1)+9]	# standard equinox (yr)
define	TEMP		Memd[PTTELDAT($1)+10]	# temp (C)
define	PRES		Memd[PTTELDAT($1)+11]	# atm. press (mm Hg)
define	PAR_ANG		Memd[PTTELDAT($1)+12]	# parallactic angle (rad)
define	WAVER		Memd[PTTELDAT($1)+13]	# wavel. for refract. (microns)
define	REF1		Memd[PTTELDAT($1)+14]	# Refract coeff, 1st order
define	REF3		Memd[PTTELDAT($1)+15]	# Refract coeff, 3rd order
define	WAVEMN		Memd[PTTELDAT($1)+16]	# min wavel. for refr (um)
define	WAVEMX		Memd[PTTELDAT($1)+17]	# max wavel. for refr (um)
define	AD1		Memd[PTTELDAT($1)+18]	# Atm disp. for WAVEMN (asec)
define	AD2		Memd[PTTELDAT($1)+19]	# Atm disp. for WAVEMN (asec)
define	AMASS		Memd[PTTELDAT($1)+20]	# Airmass
define	EPOCH		Memd[PTTELDAT($1)+21]	# Epoch of planned observation
...
define	TELESCOP	Memc[PTMSKDAT($1)+5*SZ_LINE]	# Telescope
define	BLUNAME		Memc[PTMSKDAT($1)+6*SZ_LINE]	# Blueprint name
define	GUINAME		Memc[PTMSKDAT($1)+7*SZ_LINE]	# Name to appear on GUI
define	BLUOBSR		Memc[PTMSKDAT($1)+8*SZ_LINE]	# Observer
define	USEDATE		Memc[PTMSKDAT($1)+9*SZ_LINE]	# Date of intended use
...
define	FPWIN_Z		Memr[PTFPWIN($1)+3]	# Z-sense (-1 into page)
...
# Some additional definitions, needed for Mask DataBase:

define	MDB_INSTRUME	"DEIMOS"
define	MDB_TELESCOP	"Keck II"
define	MDB_SIM_VERS	"DSimulator Ver. 1"
define	MDB_SIM_AUTH	"Phillips <phillips@ucolick.org>"

# Some additional definitions, needed for graphics
define	FLIP	 -1.	# Fudge factor for flipping X axis in graphics
			# also used (wrongly?) in dsim
define	FLDCEN_Y	270.	# Offset (asec) between tel & fld
define	XLOW_LIM	-498.		# lowest X value for slits  XXX
define	XUPP_LIM	 498.		# highest X value for slits XXX
define	YMSKMIN		187.3		# 0.5 arcsec more than "soft" limit (XX)
define	YMSKMAX		479.1		# 0.5 arcsec less than "soft" limit (XX)
define	SIM_FLD_WIDTH	1100.	# default field width for graphics
define	SIM_FLD_CENX	0.	# default field x-cen for graphics
define	SIM_FLD_CENY	180.	# default field y-cen for graphics
define	SIM_ZSENSE	-1.	# -1=Z into page, +1=Z out of page
define	FLD2TEL_PA	 HALFPI		# dPA tp Tel_Axis at Fld_Cen (rad) DMOS
define	FLD2TEL_R	 270.		# Distance to Fld_Cen (arcsec) DEIMOS
define	FLIP_RHS	 -1	# TMP FOR CLEANUP


#define	FLDCEN_Y	420.	# Offset (asec) between tel & fld
#define	XLOW_LIM	-240.		# lowest X value for slits  XXX
#define	XUPP_LIM	 240.		# highest X value for slits XXX
#define	YMSKMIN		240.5		# 0.5 arcsec more than "soft" limit (XX)
#define	YMSKMAX		599.5		# 0.5 arcsec less than "soft" limit (XX)
#define	SIM_FLD_WIDTH	800.	# default field width for graphics
#define	SIM_FLD_CENX	30.	# default field x-cen for graphics
#define	SIM_FLD_CENY	385.	# default field y-cen for graphics
#define	SIM_ZSENSE	1.	# -1=Z into page, +1=Z out of page
#define	FLD2TEL_PA	-HALFPI		# dPA tp Tel_Axis at Fld_Cen (rad) LRIS
#define	FLD2TEL_R	 420.		# Distance to Fld_Cen (arcsec) LRIS
#define	FLIP_RHS	 1	# TMP FOR CLEANUP
