...
include "align.h"
include	"futil.h"
include	"lredtest.h"		# needed only for det_to_ics

define	ROFF	90.		# rotational offset ROTPOSN--TO__PA
define DCS_HOST	"manuka"	# name of DCS machine
define DCS_SYS	"dcs1"		# name of DCS system

define		ASECPIX  0.135		# Arcsec/pixel  XXX
...
define	DEF_PROP_FILE	"lredtest$lib/prop/redprop.txt"	# default properties
...
# TMP XXX
char	mosname[SZ_FITS]
int	chipoff				# offset to correct chip number
...
begin
call eprintf ("Welcome to XBOX for LREDTEST!\n")
	zip_mode = NO

# PO is poorly defined; below it can be reset, so...
	    x0ics = INDEF
	    y0ics = INDEF
	...
	imode = clgeti ("imode")
if (imode == 3) {
	call eprintf ("WARNING: IMODE DEFAULTED to 2 (imode=3 not implemented)\n")
	imode = 2
}
...
call fatal (0, "GET_BOXES NOT YET IMPLEMENTED!!\n")
...
# call printf ("Testing what hapens to an int is: (%d) %d %d  %d %d \n")
# call pargi (i)
# call pargi (x1)
# call pargi (x2)
# call pargi (y1)
...
### NEED CHIP NUMBER
		    i = det_chip (xb, yb)
# call printf ("Image Section is: (%d) %d-%d  %d-%d \n")
# call pargi (i)
# call pargi (x1)
# call pargi (x2)
# call pargi (y1)
...
# TMP XXX if LBLUE add 2 to chip
	chipoff = 0
	if (imaccf (im0, "INSTRUME") == YES) {
		call imgstr (im0, "INSTRUME", mosname, SZ_FITS)
		if (streq (mosname, "LRISBLUE"))
			chipoff = 2
	} else {
		call fatal (0, "INSTRUME NOT DEFINED")
	}
	i = i + chipoff

# Get the image section (limit checking and zero-fill included)
#		    buf = chip_sect (x1, x2, y1, y2, mmap, i, YES)
		    buf = chip_sect (x1, x2, y1, y2, mmap, i, NO)

#  call printf (" \n ************ \n Value of first pixel in image section is: \n %f %f %f %f %f %f  \n*******")
#  call pargr (Memr[buf])
#  call pargr (Memr[buf+1])
#  call pargr (Memr[buf+2])
#  call pargr (Memr[buf+3])
#  call pargr (Memr[buf+4])
#  call pargr (Memr[buf+5])
...
# ... construct label
		    ...
		    call printf ("Box center:  %6.3f %6.3f  Star center:  %6.3f %6.3f \n")
			call pargr (xb)
			call pargr (yb)
			call pargr (xstar)
			call pargr (ystar)
...
# Close the mosaic image
	call mos_free (im0, mmap, n)	## n is nccd from mos_init
...
# Adjust all coordinates into ICS, including the POs: XXX
...
# The following is required somewhere XXX
	invert = true
...
#        ...        = 90 - (rotposn+90) (LRIS)
	...
	if (invert) {
# invert currently written to reverse y
...
# PA offset is backwards from rotation:
	...
	pdeg = RADTODEG(-atan2(coeff[1,2], coeff[1,1]))
	perr = RADTODEG(asin(err[1]))
	if (invert)			# inverted sense of rotation
		pdeg = -pdeg
	xarcsec = ASECPIX * coeff[1,3]
	yarcsec = ASECPIX * coeff[2,3]
	xerr = ASECPIX * err[2]
...
if (imode == 3) {
call fatal (0, "IMODE 3 is completely untested and you should never see this.")
...
##
## GET_FLEX: get flexure values based on INSTRUMENT-SPECIFIC MODEL
## currently set for DEIMOS. NOTE that NO correction is needed if FCS ON
## test for FCS stat is NOT implemented.
##
...
procedure	get_flex (im, xflex, yflex)

pointer	im			# image descriptor
real	xflex, yflex		# returned flexure values
...
## Find flexure correction: #  (XXX NOT NEEDED if FCS is on):
	if (imaccf (im, "ROTATVAL") == YES) {
		theta = imgetr (im, "ROTATVAL")
