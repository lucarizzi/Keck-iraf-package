...
call eprintf ("DEBUG: x,yarcs= %7.2f %7.2f\n")
call pargr (XARCS(sdat,34))
call pargr (YARCS(sdat,34))
...
## XXX The following lines are believed not needed -- check!!!
## call fatal (0, "terminate in mask_coords at bad point!")
...
call eprintf ("x,yfp= %7.2f %7.2f\n")
call pargd (xfp)
call pargd (yfp)
		call proj_to_mask (xfp, yfp, pa, xsm, ysm, pa)
call eprintf ("x,ysm= %7.2f %7.2f\n")
call pargd (xsm)
call pargd (ysm)
...
call eprintf ("%03d  %6.1f,%6.1f  %6.1f,%6.1f  %6.1f,%6.1f  %6.1f,%6.1f\n")
call pargi (i)
call pargd (XMM1(sdat,i)); call pargd (YMM1(sdat,i))
call pargd (XMM2(sdat,i)); call pargd (YMM2(sdat,i))
call pargd (XMM3(sdat,i)); call pargd (YMM3(sdat,i))
call pargd (XMM4(sdat,i)); call pargd (YMM4(sdat,i))
	}
...
call pargr (dxupp)
		...
		if (del1 != 0. && RELPA(sdat,ndx1) != INDEF) {
			...
			Y2(sdat,ndx1) = Y2(sdat,ndx1) + del1 * FLIP_RHS * tana
		}

		X1(sdat,ndx2) = X1(sdat,ndx2) - del2
		if (del2 != 0. && RELPA(sdat,ndx2) != INDEF) {
			tana = tan (RELPA(sdat,ndx2))
			Y1(sdat,ndx2) = Y1(sdat,ndx2) - del2 * FLIP_RHS * tana
		}
...
# TEST
call eprintf ("SPECIAL SECTION:\n")
		call eprintf ("\n# SLit coords:\n\n")
		do i = 0, nslit-1 {
			xas = X1(sdat,i)
			yas = Y1(sdat,i)+0.45
			call tel2radec (indat, xas, yas, ra_tv, dec_tv)
			call eprintf ("polygon (%13.3h,%13.2h,")
				call pargd (RADTODEG(ra_tv)/15.d0)
				call pargd (RADTODEG(dec_tv))
			xas = X1(sdat,i)
			yas = Y1(sdat,i)-0.45
			call tel2radec (indat, xas, yas, ra_tv, dec_tv)
			call eprintf (" %13.3h,%13.2h,")
				call pargd (RADTODEG(ra_tv)/15.d0)
				call pargd (RADTODEG(dec_tv))
			xas = X2(sdat,i)
			yas = Y2(sdat,i)-0.45
