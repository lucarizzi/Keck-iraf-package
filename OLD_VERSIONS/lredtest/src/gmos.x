...
# TBD: need to worry about number of im buffers? I don't think so, provided
# we can have as many _images_ open as we want

# tmos creates an image "testim" from hard-coded image positions, as a debug
# tool
...
# TMP
char	mosname[72]		# hardcoded FITS len XXX
...
# get: nextn, numamps, precol, postcol [pre/post row?]
	...
	if (imaccf (im0, "NVIDINP") == YES) {
		nhdu = imgeti (im0, "NVIDINP")
	} else if (imaccf (im0, "NUMAMPS") == YES) {
		nhdu = imgeti (im0, "NUMAMPS")
	} else {
		call fatal (0, "Number of HDUs cannot be determined ...")
	}
	nccd = 8				# TMP -- FIX should be var
call eprintf ("Fix NHDU/NCCD issue in MOS_INIT (esp nccd=8)\n")

	if (imaccf (im0, "INSTRUME") == YES) {
		call imgstr (im0, "INSTRUME", mosname, 72)	# XXX
	} else {
...
# ... identify region.  This should probably be done with DETSEC -- TMP?
		...
		if (streq (mosname, "LRISBLUE")) {
			chip = imgeti (im, "CCDLOC") + 1
			amp = imgeti (im, "AMPLOC") - 2*(chip-1) + 1
			chip = chip + 2
		} else {
			chip = imgeti (im, "CCDLOC")
			amp = imgeti (im, "AMPLOC") - 2*(chip-1)
		}
call eprintf ("Chip, amp= %d, %d\n"); call pargi (chip); call pargi (amp)

if (amp < 1 || chip < 1) {
	call fatal (0, "MOS_INIT: invalid chip or amp!\n")
}
		mx = chip
		my = 1
		if (chip > 4) {
...
procedure	mos_free (im0, mmap, nccd)

pointer	im0			# image pointer to primary HDU
pointer	mmap			# pointer to mosaic vectors
int	nccd			# number of CCDs in mosaic
