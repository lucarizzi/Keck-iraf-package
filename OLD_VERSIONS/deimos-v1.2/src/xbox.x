...
define	DEF_PROP_FILE	"dproto$lib/prop/redprop.txt"	# default properties
...
# ... for the PA and pointing origin
	if (imaccf (im0, "ROTPOSN") == YES) {
		...
		call eprintf ("WARNING: ROTPOSN keyword missing from image header!\n")
		call get_valr ("Please enter current rotator PA from FACSUM: ", 0., rotposn, -360., 360.)

		# take out 90 deg fudge factor (FACSUM displays ROTPOSN+90)...
		rotposn = rotposn - 90.
	}
	...
	if (imaccf (im0, "PONAME") == YES) {
		...
		call eprintf ("WARNING: PONAME keyword missing from image header; assuming DEIMOS\n")
		call strcpy ("DEIMOS", poname, SZ_FITS)
	}
...
# convert the pointing origins to ICS
    ...
    call printf ("#@ xrot=%15.5g yrot=%15.5g\n")
		call pargr( xrot)
		call pargr( yrot)
...
# Print out results
	...
	do i = 0, npt-1 {
		call printf ("# %8.2f %8.2f %8.2f (%6.2f) %8.2f (%6.2f) %6.2f\n")
