...
# GDW 2004-Aug-15 Define the plot window based on the 
# midpoint between extrema...
	...
	xf = 0.5-0.5*(xmin+xmax)/nx
	yf = 0.5-0.5*(ymin+ymax)/ny
...
begin
	...
	status = get_size( x, zx, nx, sky, cx, xsize, zxfit)
	if ( status == OK ) {
		call printf("# xsize=%g\n")
			call pargr( xsize)
	} else {
		call printf("## bad size\n")
	}
...
#-----------------------------------------------------------------------
int procedure get_size( x, z, n, zmin, xc, size, zfit)
#-----------------------------------------------------------------------
# This procedure will fit a Gaussian model to the passed marginal 
# distribution, using the builtin IRAF routine "dofit" which can be
# found in the onedspec$splot/deblend.x file.
#-----------------------------------------------------------------------

# passed params...
real	x[n]		# position vector
real	z[n]		# intensity vector
int		n			# size of vector
real	zmin		# minimum good value
real	xc			# fixed center position
real	size		# returned size
real	zfit[n]		# (returned) vector of fit values

# internals...
real	tx[n]		# work vector
real	s[n]		# error vector
real	dx			# pixel size
real	y1			# Continuum offset
real	dy			# Continuum slope
real	xp[1]		# Profile positions
real	yp[1]		# Profile intensities
real	sp[1]		# Profile Gaussian FWHM
real	lp[1]		# Profile Lorentzian FWHM
real	chisq		# Chi squared
int		i			# counter
int		fit[5]		# Fit constraints for dofit
int		nsub		# Number of subpixels
int		tp[1]		# Profile type	
...
begin
	...
	# set all error vectors to 1...
	do i=1,n {
		s[i] = 1.
	}	

	# replace values below sky...
	call amaxkr (z, zmin, tx, n)
	...
	# Do fit.
	fit[1] = 1	# background fixed
	fit[2] = 1	# position fixed
	fit[3] = 2	# intensity variable
	fit[4] = 2	# gaussian FWHM variable
	fit[5] = 2	# lorentzian FWHM variable

	# printf...
	call printf("xp=%g yp=%g sp=%g lp=%g\n")
		call pargr(xp[1])
		call pargr(yp[1])
		call pargr(sp[1])
		call pargr(lp[1])
	...
	# perform the fit...
	iferr (call dofit (fit, x, tx, s, n, dx, nsub, y1, dy, 
		xp, yp, sp, lp, tp, np, chisq)) {
		return (ERR)
	} 
