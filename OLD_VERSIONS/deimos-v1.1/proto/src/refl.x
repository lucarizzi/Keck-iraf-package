...
# 6. Convert to x,y in focal plane:
	rp = sqrt (xp*xp + yp*yp)
	foc = CAM_FOC(sys)	# this is independent of wavelength!
...
procedure	grat_to_ics (beta, gamma, wave, sys, a, ag, as, x, y)

double	beta, gamma			# returned angles
double	wave				# lambda in microns (for focus)
double	sys[NPARAM]			# system parameters
double	a[3,3]				# camera transformation
double	ag[3,3]				# grating transformation
double	as[3,3]				# slider transformation
...
#
# CHROM_FOC: focus with allowance for chromatic adjustment
#	The following is approx true: (1.-3.78e-4/(w*1.e-4-0.2))
#

double	procedure chrom_foc (wave, sys)
...
begin
	w = abs (wave)
	focus = CAM_FOC(sys) * (1.d0 - 3.78d-4/(w-0.2))
	return (focus)
end
