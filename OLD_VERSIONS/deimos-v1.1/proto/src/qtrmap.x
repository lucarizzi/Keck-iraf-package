...
#Note: these are added, but how to employ??
call eprintf ("REMINDER: need to decide on how to treat SLIDER ERRORS\n")
...
do i = 1, 8 {
	...
	call qtrmap (fda, fdb, e1, a2, as, a3, a4, ccd, sys)
...
procedure	qtrmap (fda, fdb, e1, a2, as, a3, a4, ccd, sys)
...
double	scaling			# scaling 19C to 2C
...
call eprintf ("CAVE!! TEST!! SCALING for thermal x,ymm -- NOT APPLIED TO SKY!!\n")
scaling = 1.0d0 - 2.0d-5 * 17.
	do j = -nex, ny+nex {
		y = j * ystep + yoff
		do i = -nx-nex, nx+nex {
			x = i * xstep
			if (mask)
				call pre_grating (x*scaling, y*scaling, e1, a2, as, sys, r)
			else
				call sky_grating (x, y, e1, a2, as, sys, r)
			tanx = (-r[1] / -r[3])
			tany = (-r[2] / -r[3])
...
# Write the bmap
	...
	do j = -ny-nex, ny+nex {
		...
		do i = -nx-nex, nx+nex {
			...
			if (x*x+y*y > (5080.**2))
				next
			call ics_post_grating (x, y, a4, as, sys, r)
			tanx = (-r[1] / -r[3])
			tany = (-r[2] / -r[3])
			if (reverse) {
...
procedure	ics_post_grating (x, y, a, as, sys, r)

double	x, y
double	a[3,3]				# camera tranformation
double	as[3,3]				# slider tranformation
double	sys[NPARAM]			# system parameters
...
# New -- transform into slider system
...
procedure	sky_grating (xarcs, yarcs, e1, a2, as, sys, r)
