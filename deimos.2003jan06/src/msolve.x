...
# ADDED
	XFO_TLT(sys) = 0.
	YFO_TLT(sys) = 0.	# -1.72e-4
call eprintf ("Special Y_TILT ADDED!\n")
...
# ADDED
	call fprintf (fdd, "# XFO_TLT: %9.3g;  YFO_TLT: %9.3g \n")
		call pargd (XFO_TLT(sys))
...
procedure multi_solve (xs, ys, w, xobs, yobs, xadj, yadj, x, y, nslit, niter, ccd, sys, rmsx, rmsy, nset, spar, setndx, mat, px, py, del, fix, nv, nc)
...
# ADDED
		call numderiv (XFO_TLT(sys), delang, xs[k], ys[k], w[k], asi,
			px[FX], py[FX], ccd, sys, e1, a2, a3, a4, fix[FX])

		call numderiv (YFO_TLT(sys), delang, xs[k], ys[k], w[k], asi,
			px[FY], py[FY], ccd, sys, e1, a2, a3, a4, fix[FY])
...
# ADDED
	    delta = mat[nc,FX]
	    del[FX] = 0.79 * max (min (delta, delang), -delang)

	    delta = mat[nc,FY]
	    del[FY] = 0.79 * max (min (delta, delang), -delang)
...
# Update parameters
	    ...
	    XFO_TLT(sys) = XFO_TLT(sys) + del[FX]
	    YFO_TLT(sys) = YFO_TLT(sys) + del[FY]
...
# store parameter
	...
	do i = -noff, noff {
		varpar = par0 + vinc * i
		call setup (e1, a2, a3, a4, ccd, sys)
		call pt_xfm (xs, ys, w, e1, a2, a3, a4, ccd, sys, x[ndx], y[ndx], n)
		ndx = ndx + 1
	}
...
procedure numder_ccd (varpar, delpar, xs, ys, w, asi, px, py, ccd, sys, e1, a2, a3, a4, fixed)
...
# Solve for ICS location
	call setup (e1, a2, a3, a4, ccd, sys)
	call pt_xfm (xs, ys, w, e1, a2, a3, a4, ccd, sys, xics, yics, n)
...
# Now work with CCDs -- note that only ccd_geom must be called now ...
	...
	do i = -noff, noff {
		varpar = par0 + vinc * i
		call ccd_geom (ccd, sys, NO)
		call ics_to_ccd (xics, yics, ccd, n, x[ndx], y[ndx])
		if (mosim_coord(x[ndx], y[ndx], n) == OFF_CHIP) {
			x[ndx] = abs (x[ndx])
			y[ndx] = abs (y[ndx])
		}
