...
# Read in params
	...
	PA_FLD(indat) = DEGTORAD(clgetd ("PA0"))
	...
	call strcpy (MDB_SIM_VERS, DESCREAT(indat), SZ_LINE)
	call strcpy (MDB_INSTRUME, INSTRUME(indat), SZ_LINE)
	call strcpy (MDB_TELESCOP, TELESCOP(indat), SZ_LINE)
...
# Is slitwidth / lengths / PA present, or assume default?
		if (n < 13) {
			slitwid = DEF_SLWID(indat)
		}
		if (n < 12) {
			l1 = DEF_HLEN(indat)
			l2 = DEF_HLEN(indat)
...
#
# FLD2TELAX:  from field center and PA, calc coords of telescope axis
# Requires fixed delta-PA from PA-ref to Tel_Axis at Fld_Center
# (different than before -- adopts Fld PA & assumes DCS knows correct PA_ROT)
# (difference at dec=-36 is about 0.25 arcsec for DEIMOS)
# Note that we must calculate correct PA_ROT, as this is used in tel_coords
...
double	cosd0, sind0, cosb, sinb

begin

# convert field center offset (arcsec) to radians
	r = DEGTORAD (FLD2TEL_R / 3600.)

# get PA to tel_ax at field center
	pa_telax = PA_FLD(indat) + FLD2TEL_PA
	...
	sind0 = sind * cosr + cosd * sinr * cost
	cosd0 = sqrt (1. - sind0*sind0)
	sina = sinr * sint / cosd0		# ASSUME not at dec=90
	cosa = sqrt (1. - sina*sina)
	sinb = cosd * sint / cosd0
	cosb = -cosa * cost + sina * sint * sind
	RA_TEL(indat) = RA_FLD(indat) + asin (sina)
	DEC_TEL(indat) = asin (sind0)
	PA_ROT(indat) = PI - (FLD2TEL_PA + atan2 (sinb, cosb))

call eprintf ("DEBUG: RA,DEC: %10f %10f %10f %10f\n")
...
#		r = RADTODEG(r) * 3600.
		...
		XARCS(tdat,i) = r * cos (p - pa0)		# TEST!!
		YARCS(tdat,i) = r * sin (p - pa0) * FLIP_RHS	# TEST!!
...
# For simplicity, we calculate the endpoints in X here; note use of FLIP
		...
		ygeom = sin (rangle)
		xgeom = cos (rangle)				# TEST!!
		ygeom = FLIP_RHS * sin (rangle)			# TEST!!
...
# include "chk_stat.dmos"
include "chk_stat.lris"
...
# XXX
	...
	call pkwstr (fd, kdat, "AUTHOR", MDB_SIM_AUTH, snul)
	call pkwstr (fd, kdat, "CREATOR", MDB_SIM_VERS, snul)
